use self::participant::ParticipantConnection;
use self::room::RoomId;
use self::rooms_registry::RoomsRegistry;
...
mod participant {
    ...
    pub struct ParticipantConnection {
        ...
        plain_video: PlainTransport,
        plain_audio: PlainTransport,
    }
    ...
    impl ParticipantConnection {
        ...
        pub async fn new(room: Room) -> Result<Self, String> {
            ...
            let (plain_audio, plain_video) = {
                let mut plain_opt = PlainTransportOptions::new(ListenInfo {
                    protocol: Protocol::Udp,
                    ip: IpAddr::V4(Ipv4Addr::LOCALHOST),
                    announced_ip: None,
                    port: None,
                ...
                plain_opt.rtcp_mux = false;
    
                let plain_audio = room.router()
                .create_plain_transport(plain_opt.clone()).await
                .map_err(|error| format!("Failed to create plain audio: {error}"))?;
    
                plain_audio.connect(PlainTransportRemoteParameters {
                    ip: Some("127.0.0.1".parse().unwrap()),
                    port: Some(1232),
                    rtcp_port: Some(1233),
                    srtp_parameters: None,
                }).await
                .map_err(|error| format!("Failed to connect plain audio: {error}"))?;

                let plain_video = room.router()
                .create_plain_transport(plain_opt).await
                .map_err(|error| format!("Failed to create plain video: {error}"))?;
    
                plain_video.connect(PlainTransportRemoteParameters {
                    ip: Some("127.0.0.1".parse().unwrap()),
                    port: Some(1234),
                    rtcp_port: Some(1235),
                    srtp_parameters: None,
                }).await
                .map_err(|error| format!("Failed to connect plain video: {error}"))?;

                (plain_audio, plain_video)
            };
        ...
        fn handle(&mut self, message: ClientMessage, ctx: &mut Self::Context) {
            match message {
                ...
                ClientMessage::Produce {
                    ...
                    let plain_video = self.plain_video.clone();
                    let plain_audio = self.plain_audio.clone();
                            ...
                            .await
                        {
                            Ok(producer) => {
                                if kind == MediaKind::Video {
                                    let r: Result<(), String> = consume_plain_video(&address, producer.id(), &plain_video).await;
                                    println!("consume_plain_video result: {r:?}");
                                } else if kind == MediaKind::Audio {
                                    let r = consume_plain_audio(&address, producer.id(), &plain_audio).await;
                                    println!("consume_plain_audio result: {r:?}");
                                }
    ...
    async fn consume_plain_video(addr: &Addr<ParticipantConnection>, producer_id: ProducerId, plain_video: &PlainTransport) -> Result<(), String> {
        let options = ConsumerOptions::new(producer_id, RtpCapabilities {
            codecs: vec![
                RtpCodecCapability::Video { 
                    mime_type: MimeTypeVideo::H264,
                    preferred_payload_type: None, // Some(96),
        ...
        let consumer = plain_video.consume(options).await
        .map_err(|e|format!("consume failed: {e:?}"))?;

        addr.do_send(InternalMessage::SaveConsumer(consumer));

        Ok(())
    }

    async fn consume_plain_audio(addr: &Addr<ParticipantConnection>, producer_id: ProducerId, plain_audio: &PlainTransport) -> Result<(), String> {
        let options = ConsumerOptions::new(producer_id, RtpCapabilities {
            codecs: vec![
        ...
        let consumer = plain_audio.consume(options).await
        .map_err(|e|format!("consume audio failed: {e:?}"))?;
