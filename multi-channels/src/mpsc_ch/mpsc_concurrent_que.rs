    ...
    T: Clone,
{
    ...
    fn channel(cap: usize) -> (Self::Sender, Self::Receiver) { 
        let shared = Arc::new(Shared {
            que: ConcurrentQueue::bounded(cap),
    ...
    fn try_send(&mut self, msg: T) -> Result<(), TrySendError<T>> { 
        ...
        match r {
            ...
            Err(v) => { 
                ...
                match v {
                    concurrent_queue::PushError::Full(v) => Err(TrySendError(v)),
                    concurrent_queue::PushError::Closed(v) => Err(TrySendError(v)),
                }
            },
        }
    }
    ...
    T: Clone,
{
    fn try_recv(&mut self) -> Result<T, TryRecvError> { 
        ...
        match r {
            ...
            Err(e) => {
                match e {
                    concurrent_queue::PopError::Empty => Err(TryRecvError::Empty),
                    concurrent_queue::PopError::Closed => Err(TryRecvError::Empty),
                }
            },
        }
    }
